...
from .migration_helpers import read_file_through_api, download_ibsolution, \
    download_dependencies_from_dev_and_upload_to_prod, get_dependencies_from_solution_build_folder
...
def migrate_solution(source_ib_host, target_ib_host, source_api_token, target_api_token, solution_build_dir_path,
    ...
    ibsolution_dir = os.path.dirname(solution_build_dir_path)
    ibsolution_file_path = os.path.join(ibsolution_dir, f'{package["name"]}-{package["version"]}.ibsolution')

    # Download ibsolution from dev environment
    resp = download_ibsolution(source_ib_host, source_api_token, ibsolution_file_path, False)
    ...
    upload_file(target_ib_host, target_api_token, upload_path, resp.content)

    # Get dependencies (dev packages + model solutions) from package.json
    requirements_dict = get_dependencies_from_solution_build_folder(source_ib_host, source_api_token, solution_build_dir_path)
    ...
    # Publish ibsolutions to target marketplace
    for ib_solution_path in uploaded_ibsolutions:
        publish_resp = publish_to_marketplace(target_ib_host, target_api_token, ib_solution_path)
...
def migration_main(**kwargs):
    fn_context = kwargs.get('_FN_CONTEXT_KEY')
    config, err = fn_context.get_by_col_name('CONFIG')
    clients, err = fn_context.get_by_col_name('CLIENTS')

    # migration_config_file = "rktlogicdocs/UAT/fs/Rocket S3/CICDTesting/CICDFlow/modules/1040_config.json"
    migration_config_file = config["migration_config_path"]
    migration_config_bytes, err = clients.ibfile.read_file(migration_config_file)
    migration_config = json.loads(migration_config_bytes)

    source_ib_host = migration_config["SOURCE_IB_HOST"]
    target_ib_host = migration_config["TARGET_IB_HOST"]
    source_api_token = migration_config["SOURCE_API_TOKEN"]
    target_api_token = migration_config["TARGET_API_TOKEN"]
    solution_build_dir_path = migration_config["SOLUTION_BUILD_DIR_PATH"]
    target_ib_solution_folder = migration_config["TARGET_IB_SOLUTION_FOLDER"]
    source_download_folder_dir = migration_config["SOURCE_DOWNLOAD_FOLDER_PATH"]
    target_upload_folder_dir = migration_config["TARGET_UPLOAD_FOLDER_DIR"]

    migrate_solution(source_ib_host, target_ib_host, source_api_token, target_api_token, solution_build_dir_path,
                     target_ib_solution_folder, source_download_folder_dir, target_upload_folder_dir,
                     **kwargs)
