...
def parse_dependencies(package_dependencies):
  models = {m.split('==')[0].strip(): m.split('==')[1].strip() for m in package_dependencies.get('models', [])}
  packages = {p.split('==')[0].strip(): p.split('==')[1].strip() for p in
              package_dependencies.get('dev_exchange_packages', [])}
...
def get_latest_ibsolution_path(api_token, files_api, solution_path):
  ...
  paths = [s['full_path'] for s in nodes['nodes'] if s['full_path'].endswith('.ibsolution')]
...
def read_target_package():
  ...
  path_to_ib_solution = get_latest_ibsolution_path(TARGET_IB_API_TOKEN, TARGET_FILES_API, TARGET_IB_PATH)
  unzip_files(TARGET_IB_HOST, TARGET_IB_API_TOKEN, path_to_ib_solution,
              new_path)  # TODO: Does this unzip work on a non-zip file path like ib_solution_path?
