...
def upload_chunks(ib_host, path, api_token, file_data):
    ...
    with bytes_io_content as f:
        ...
        for chunk in iter(lambda: f.read(part_size), b''):
            ...
            resp = requests.patch(append_root_url, headers=headers, data=chunk, verify=False)
            part_num += 1
...
def upload_file(ib_host, api_token, file_path, file_data):
    ...
    if resp.status_code != 204:
        raise Exception(f'Upload file failed: {resp.content}, url: {url}')
...
def package_solution(ib_host, api_token, content_folder, output_folder):
    ...
    resp = requests.post(create_solution_url, headers=headers, data=json_data, verify=False)

    # Verify request was completed successful
    content = json.loads(resp.content)
    if resp.status_code != 200 or ('status' in content and content['status'] == 'ERROR'):
...
def unzip_files(ib_host, api_token, zip_path, destination_path=None):
    ...
    destination_path = destination_path if destination_path else '.'.join(zip_path.split('.')[:-1])
    headers = {'Authorization': 'Bearer {0}'.format(api_token)}
...
def compile_solution(ib_host, api_token, solution_path, relative_flow_path):
    ...
    data = json.dumps({
        ...
        'flow_project_root': os.path.join(solution_path, *relative_flow_path.split('/')[:-1]),
        'predefined_binary_path': os.path.join(solution_path, bin_path),
        'settings': {'flow_file': relative_flow_path.split('/')[-1]}
    })
    resp = requests.post(url.replace('//d', '/d'), headers=headers, data=data, verify=False)
...
def copy_file_within_ib(ib_host, api_token, source_path, destination_path, use_clients=False, **kwargs):
    """
...
def read_file_content_from_ib(ib_host, api_token, file_path_to_read, use_clients=False, **kwargs):
    """
...
def delete_folder_or_file_from_ib(path_to_delete, ib_host=None, api_token=None, use_clients=False, **kwargs):
    """
...
def list_directory(ib_host, api_token, directory):
    """
    Lists a directory to get all content nodes
    (https://www.instabase.com/docs/apis/v2-apis/filesystem-v2/#list-directory)

    :param ib_host: (string) IB host url (e.g. https://www.instabase.com)
    :param api_token: (string) API token for IB environment
    :param directory: (string) path to directory to list
    :return: List of nodes
    """
    file_api_root = __get_file_api_root(ib_host)
    headers = {'Authorization': 'Bearer {0}'.format(api_token)}
    url = os.path.join(file_api_root, directory)

    node_list = []
    start_token = 0
    while start_token != -1 and start_token != '':
        # Make list directory request
        ...
        # If status code isn't 200 log error
        if resp.status_code != 200:
            raise Exception('Issue with list directory for {}: {}'.format(directory, content))

        # Add nodes to list
        node_list += content['nodes']

        # Continue with new token if one exists
        if content['has_more']:
...
def search_jobs_within_time_range(ib_host, api_token, limit=20, offset=0, from_timestamp=None, to_timestamp=None,
                                  state=None, user=None, tags=None, pipeline_ids=None, review_state=None,
                                  job_id=None, job_ids=None, reviewer=None, **kwargs):
    file_api_root = __get_file_api_root(ib_host, api_version="v1", add_files_suffix=False)
    headers = {'Authorization': 'Bearer {0}'.format(api_token)}
    url = f"{file_api_root}/jobs/list"
    ...
    args_to_add = []
    for arg_label, arg_value in request_arguments.items():
        if arg_value is not None:
            args_to_add.append("{}={}".format(arg_label, arg_value))

    if args_to_add:
        url += "?" + "&".join(args_to_add)

    resp = requests.get(url, headers=headers, verify=False)
    content = json.loads(resp.content)

    if not (isinstance(content, dict) and 'jobs' in content):
        logging.error("Invalid call to {}, message: {}".format(url, content))
        return None
    return content
